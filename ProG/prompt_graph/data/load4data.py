...
def load4link_prediction_single_graph_modified(path=None, num_per_samples=1):
    #load4link_prediction_single_graph 但是扰动过的图
    if path is None:
        raise ValueError("path must be provided for modified graph loading.")   
    ...
    try:
        ...
        features = sp.csr_matrix(
            (data_npz['attr_data'], data_npz['attr_indices'], data_npz['attr_indptr']),
            shape=data_npz['attr_shape']
        )
        labels = data_npz['labels']
        idx_train = data_npz['idx_train']
        idx_val = data_npz['idx_val']
    except Exception as e:
        print(f"Error accessing npz file: {e}")
        print("Detailed information about each key:")
        for key in data_npz.keys():
            try:
                value = data_npz[key]
                shape_info = f"Shape: {value.shape}" if hasattr(value, 'shape') else "No shape attribute"
                type_info = f"Type: {type(value)}"
                print(f"  {key}: {type_info}, {shape_info}")
                if isinstance(value, np.ndarray) and value.size < 10:
                    print(f"    Value: {value}")
            except Exception as inner_e:
                print(f"  {key}: Error accessing value: {inner_e}")
        raise
    idx_test = data_npz['idx_test']
    ...
    return data, edge_label, edge_index_all, input_dim, output_dim
