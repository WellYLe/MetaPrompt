...
此外，您可以直接使用我们的预训练 GNN，或使用我们的预训练模块，通过以下方式预训练您想要的 GNN 
```shell
pip install torch_cluster  -f https://data.pyg.org/whl/torch-2.3.0+cu121.html
```
torch和 Cuda 版本可以参考 https://data.pyg.org/whl/  
...
首先，从 onedrive 下载 https://1drv.ms/u/s!ArZGDth_ySjPjkW2n-zsF3_GGvC1?e=rEnBA7 (126MB) `Experiment.zip`.
您可以解压缩以获取我们的数据集预训练模型，该模型已经预训练，并且在少样本设置中 induced graph, sample data. (请确保解压缩后的文件夹名称为 `/Experiment`)
如果下载链接不可用，请随时联系我们(barristanzi666@gmail.com)

**警告！数据集提供者可能会更新数据集本身，导致与我们提供的预训练模型不兼容。我们发现 ENZYMES, BZR 数据集的实验结果可能会受到影响。**
